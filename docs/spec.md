...
- **審査メンバー**: 採点できるのは登録された審査メンバーのみ（`interview_reviewers` テーブル）。管理者が `/reviewer_add`・`/reviewer_remove`・`/reviewer_list` で登録・解除・一覧する。
- **採点**: 登録された審査メンバーが4項目（プロフとの整合性・聞き取りやすさ/イケボ・トーク力・人柄、各0〜2点）を RadioGroup モーダルで採点。0点をつけたら理由が必須（0点なしなのに理由があると弾く）。
- **審査結果**: 登録された審査メンバー人数と同数の採点が集まったら、平均点・合否（合計5点以上で合格）・0点をつけた人（誰が/どの項目/理由）を Components V2 の区切り線付きパネルで、管理者メンション付きで投稿。審査メンバーが1人も登録されていない間は結果を出さない。
- **採点状況**: 審査メッセージの「採点状況」ボタンで、登録審査メンバーの採点済み・未採点を確認できる。
- **期限管理（`review_deadline_loop`・10分間隔）**: 提出から**12時間**で未採点の審査メンバーへメンションで催促。提出から**21時間**（案内する24時間期限の3時間前）で、集まった採点だけで強制的に結果を出す（採点が0件なら管理者へ通知）。転送済みの審査は `interview_reviews` テーブルで管理。
