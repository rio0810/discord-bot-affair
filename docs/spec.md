...
- `TRIAL_WARNING_SOUND` … お試し通話 残り5分で鳴らすサウンド（IDまたは名前）
...
- **お試し通話**: VC入室から30分で自動終了。残り5分でメンション警告＋Botが一時的にVCへ入りサウンドボードを鳴らす（`TRIAL_WARNING_SOUND`）。同じ相手には1回まで（`trial_invites`）。
...
- `call_matching.py` … お試し通話の残り時間監視（1分ごと・残り5分で警告、30分で終了）
