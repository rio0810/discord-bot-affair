...
| `RECORDING_FORUM_CHANNEL_ID` | Common 審査 forum: post as a new thread titled with the submitter's name (priority over `RECORDING_FORWARD_CHANNEL_ID`). Used when the gendered ones below are unset |
| `RECORDING_FORUM_MALE_ID` / `RECORDING_FORUM_FEMALE_ID` | Per-gender 審査 forums. Male reviews go to MALE, female to FEMALE; falls back to `RECORDING_FORUM_CHANNEL_ID` then the text channel |
